...
def get_host_per_subnet(subnet_mask, network_class):
    ...
    if network_class == "C":
        return str(2 ** "".join(subnet_mask[2:3]).count("0") - 2)
...
def get_subnet_stats(ip_addr,subnet_mask):
   ...
   print("Address: {}/{}".format(ip_addr, cidr_notation))
   ...
   print("Addressable hosts per subnet: {}".format(number_of_hosts))
   ...
   print("First addresses: {}".format(str(first_address)))
...
def get_class_stats(ip_addr):
    ...
    #Finds the clas of the network inputted and stores it in a variable so it can be used in other tasks.
    network_class = get_network_class(each_byte[0])
    print("Class: " + network_class)

    #Determines if the class is less than C and then retrieves the information from the dictionary and formats it to be output.
    if network_class <= "C":
        print("Network: " + str(2 ** classes[network_class]["network_bits"]))
        print("Host: " + str(2 ** classes[network_class]['host_bits']))

    #Determines the class is D or E and then retrieves the information from the dictionary and formats it to be output.
    ...
    #Retrieve information from the dictionary and reformats to output.
